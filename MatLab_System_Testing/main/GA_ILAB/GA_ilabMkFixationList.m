function  GA_ilabMkFixationList(idx)
...
%  In this case the algorithm initializes a window over the first
...
% fix.list = [trialnum xCtr yCtr xShiftDir dShift fixStartIndex fixDurationIndex pctInvalid];
% ___________________________________________________________________________

% Authors: Roger Ray, Darren Gitelman
% $Id: ilabMkFixationList.m 1.8 2004-02-17 23:21:31-06 drg Exp $
...
switch eye.AP.fix.type
    case 'vel'
        hMax = eye.AP.fix.params.vel.hMax;
        vMax = eye.AP.fix.params.vel.vMax;
        mindur = eye.AP.fix.params.vel.minDuration/acqIntvl;
    case 'disp'
        MaxDisp = eye.AP.fix.params.disp.Disp;
        ...
        mindur = ceil(eye.AP.fix.params.disp.minDuration/acqIntvl);
        nandur = eye.AP.fix.params.disp.NaNDur;
end
...
for n = 1:nTrials
    ...
    %     GA_ilabProgressBar('update',100*n/nTrials,...
        ...
        case 'vel'
            ...
            if ~((fix1(k)==0 && fix2(k)==0))
                ...
                for j = 1:size(fixIdx,1)
                    ...
                    list = [list; n hpt vpt xShiftDir dShift fixStartIdx fixDurIdx pctInvalid];
                end
            end
        case 'disp'
            lasthpt = [];
            ...
            while i < size(trial,1)-mindur
                ...
                if i > drawthresh
                    drawthresh = drawthresh+100;
                    GA_ilabProgressBar('update',100*i/size(trial,1),...
                ...
                if any(trialnan(i,1:2))
                    nanidx = find(trialnan(i:end,1) == 0, 1, 'first');
                    if isempty(nanidx)
                        break;
                    else
                        i = i + nanidx -1;
                ...
                if i <= size(trial,1) - mindur
                    ...
                    while (disp <= MaxDisp)
                        if j > drawthresh
                            drawthresh = drawthresh+100;
                            GA_ilabProgressBar('update',100*j/size(trial,1),...
                                ['Calculating Fixations at trial: ',num2str(n),...
                                ',  datapoint: ' num2str(j)]);
                        end;
                ...
                if (j - i + 1 >= mindur)
                    ...
                    % ---------------------------------------------------------------
                    list = [list; n meanh meanv xShiftDir dShift i j-i+1 pctInvalid];
...
%  Store the list of fixation results
eye.AP.fix.list = list;

%  Make a readable table from the list
eye.AP.fix.table = GA_ilabMkFixationTbl(eye.AP.fix.list, 'listbox');
